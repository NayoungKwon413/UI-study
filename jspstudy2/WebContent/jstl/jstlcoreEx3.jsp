...
<c:set var="sum" value="${0}" />  <%-- sum 을 0으로 초기화 --%>
<h3>forEach 태그를 이용하여 1부터 100까지 짝수 합 구하기</h3>
...
<c:set var="sum" value="${0}" />  <%-- sum 을 0으로 초기화 --%>
<h3>if 와 forEach 태그를 이용하여 1부터 100까지 짝수 합 구하기</h3>
<c:forEach var="i" begin="1" end="100">
	<c:if test="${i%2 == 0}">
	<c:set var="sum" value="${sum+i}" />
	</c:if>
...
<h3>forEach 태그를 이용하여 1부터 100까지 홀수 합 구하기</h3>
...
<c:set var="sum" value="${0}" />
<h3>if 와 forEach 태그를 이용하여 1부터 100까지 홀수 합 구하기</h3>
<c:forEach var="i" begin="1" end="100">
	<c:if test="${i%2 == 1}">
	<c:set var="sum" value="${sum+i}" />
	</c:if>
...
<h3>forEach 태그를 이용하여 구구단 출력하기(중첩for문)</h3>
...
%>
<c:forEach var="i" items="${list}">  <%-- items="${list}" : list는 pageContext에 등록된 속성의 이름 --%>
	${i}&nbsp;&nbsp;
</c:forEach>

<c:forEach var="i" items="${list}" varStatus="stat">  <%-- varStatus="stat" : 몇 번째 돌고 있는지 확인--%>
	<c:if test="${stat.index==0}">  <%-- ${stat.index==0} : 첫번째 요소인가? --%>
