...
contains(str1, str2)=${fn:contains(str1, str2)}  : str1 안에 str2 문자열이 있으면 true, 없으면 false<br>
trim(str1)=${fn:trim(str1)} : 공백을 제거<br>
replace(str1, " ", "-")=${fn:replace(str1, " ", "-")} : " "을 "-" 문자열로 치환<br>
split(tokens,',') : 콤마를 기준으로 문자열을 나눠서 배열로 리턴<br>
<c:set var="arr" value="${fn:split(tokens,',')}" />
<c:forEach var="i" items="${arr}">
	${i}&nbsp;
</c:forEach><br>
join(arr, '-')=${fn:join(arr, '-')} : arr 배열을 '-' 하이픈으로 묶음<br>
</body>
</html>